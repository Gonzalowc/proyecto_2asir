...
if(sessionStorage.getItem("rol") == "SuperAdmin"){
  $queryString ='SELECT u.idusuario, u.nombre, u.correo, u.usuario, r.rol FROM usuario u INNER JOIN rol r ON r.idrol=u.rol WHERE estatus=? ORDER BY u.idusuario'
  jefe = 1;
}else{
  $queryString =
    ...
    $(".link_edit").click(function() {
      ...
      recuperar +='<div class="alertModal">';
	    ...
	    recuperar +='</div>';
			...
			recuperar +='			<input id="usuario" name="Usuario" value="'+Usuario+'"></input>';
			recuperar +='		<label for="Correo">Correo</label>';
			recuperar +='			<input id="correo" name="Correo" value="'+Correo+'"></input>';
      ...
      $(".btn_save").click(function() {
        ...
        $queryString ="UPDATE usuario set nombre=?, correo=?, usuario=?, rol=? where idusuario= ?";
        
        connection.query($queryString, [Nombre, Correo, Usuario, Rol, ID], (err, results) => {
          if (err) {
            return console.log("An error ocurred with the query", err);
          }
          if(results){
