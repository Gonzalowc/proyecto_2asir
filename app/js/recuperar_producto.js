...
var list_table = "";
  $queryString =
    "SELECT p.codproducto, p.producto, p.descripcion, pd.proveedor, p.precio, p.existencia, p.foto FROM producto p join proveedor pd on p.proveedor = pd.codproveedor WHERE p.estatus=0 ORDER BY codproducto";
...
function click() {
  $(document).ready(function () {
    $(".link_delete").click(function () {
      ...
      recuperar += "		<h2>Recuperar Producto</h2>";
      ...
      recuperar += '			<button type="button" class="btn_save">Recuperar</button>';
      recuperar += '		</div>';
      recuperar += '	</div>';
      recuperar += '</section>';
      ...
      $(".btn_save").click(function () {
        ...
        connection.query($queryString, [idusu], (err, results) => {
          ...
          if (results[0].idusuario == idusu && results[0].clave == clave) {
            $queryString =
              "UPDATE producto set estatus = 1 where codproducto= ?";
